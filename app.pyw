...
from PyQt5.QtWidgets import (QApplication, QWidget, QVBoxLayout, QTextEdit,
                             ...
                             QFileDialog, QSlider, QDialog, QLabel, QPushButton, QHBoxLayout)
from PyQt5.QtGui import QFont, QKeyEvent, QKeySequence, QCloseEvent, QTextCursor, QTextOption
...
class Main(QWidget):
    ...
    def init_menu(self):
        ...
        self.settings.triggered.connect(self.open_settings_json)
    ...
    def open_settings_json(self):
        if not self.is_saved:
            reply = self.ask_yes_no("File is not saved, Save it?")
        ...
        self.path = f"{getcwd()}\\config.json"
        self.is_saved = True
        self.text_area.setText(open(f"{getcwd()}\\config.json", 'r').read())
        QMessageBox.information(self, "App", "After making changes to the settings, Please restart the editor")
    
    def toggle_wordwrap(self):
        if self.text_area.wordWrapMode() == QTextOption.WrapMode.WrapAtWordBoundaryOrAnywhere:
    ...
    def init_pref_menu(self):
        ...
        toggle_wordwrap.triggered.connect(self.toggle_wordwrap)
    ...
    def init_insert_menu(self):
        ...
        insert_completion_menu = QMenu('Completion', self)
        
        insert_completed = QAction('Insert Completed', self)
        insert_Dictionary = QAction('Insert Dictionary', self)
        insert_English = QAction('Insert English', self)
        insert_Character_says = QAction('Insert Character said', self)
        insert_There_were = QAction('Insert There were', self)
        
        # Set Shortcuts #
        
        insert_completed.setShortcut(QKeySequence(
            Qt.Modifier.ALT |
            Qt.Modifier.SHIFT |
            Qt.Key.Key_C
        ))
        insert_Dictionary.setShortcut(QKeySequence(
            Qt.Modifier.ALT |
            Qt.Modifier.SHIFT |
            Qt.Key.Key_D
        ))
        insert_English.setShortcut(QKeySequence(
            Qt.Modifier.ALT |
            Qt.Modifier.SHIFT |
            Qt.Key.Key_E
        ))
        insert_There_were.setShortcut(QKeySequence(
            Qt.Modifier.ALT |
            Qt.Modifier.SHIFT |
            Qt.Key.Key_T
        ))
        insert_Character_says.setShortcut(QKeySequence(
            Qt.Modifier.ALT |
            Qt.Modifier.SHIFT |
            Qt.Key.Key_S
        ))
        ...
        insert_completed.triggered.connect(lambda: self.insert_completion('completed '))
        insert_Dictionary.triggered.connect(lambda: self.insert_completion('dictionary '))
        insert_English.triggered.connect(lambda: self.insert_completion('english '))
        insert_Character_says.triggered.connect(lambda: self.insert_completion(' said ', 6))
        insert_There_were.triggered.connect(lambda: self.insert_completion('there were '))
        
        #############
        
        insert_completion_menu.addActions([
            insert_completed, insert_Dictionary,
            insert_English, insert_There_were,
            insert_Character_says
        ])
        
        # Add user defined completions #
        
        completions = list(self.pref.get('completions').values())
        print(completions)

        import functools
        
        for completion_and_shortcut in completions:
            completion, shortcut = completion_and_shortcut
            print(completion)
            shortcut = f"Alt+Shift+{shortcut}" if shortcut != '' else None
            
            action = QAction(f"Insert {completion}", self)
            if shortcut:
                action.setShortcut(shortcut)
            print(completion)
            action.triggered.connect(functools.partial(lambda comp: self.insert_completion(comp), completion))
            
            insert_completion_menu.addAction(action)
        
        ################################
        
        self.insert_menu.addMenu(insert_completion_menu)
        
        self.menu.addMenu(self.insert_menu)
    
    # Insert Completion #
    
    def insert_completion(self, completion: str, move_by: int = 0):
        c = self.text_area.textCursor()
        
        c.insertText(completion)
        
        if move_by != 0:
            c.movePosition(QTextCursor.MoveOperation.Left, QTextCursor.MoveMode.MoveAnchor, move_by)
        ...
        if e.key() == Qt.Key.Key_QuoteDbl:
            ...
            try:
                ...
                if c.hasSelection():
                    ...
                    elif c.selectedText() == 'ltlr':
                        self.insert_completion_to_area(c, 'literally ')
                        c.movePosition(QTextCursor.MoveOperation.Left, QTextCursor.MoveMode.MoveAnchor, 6)
                        self.setTextCursor(c)
                    
                    elif c.selectedText() in list(self.p.pref.get('completions', {}).keys()):
                        keys = list(self.p.pref.get('completions', {}).keys())
                        completion = keys[keys.index(c.selectedText())]
                        completion = self.p.pref.get('completions', {}).get(completion)[0]
                        
                        self.insert_completion_to_area(c, completion)
